...
sig
  ...
  (**
   * returns the value from the (value, key) pair with the maximum
   * key, for example `from e in emps group e.deptno compute (argMax
   * over (e.ename, e.sal))`.
   *)
  val argMax : ('a * 'b) bag -> 'a [@@prototype "argMax bag"]

  (**
   * returns the value from the (value, key) pair with the minimum
   * key.
   *)
  val argMin : ('a * 'b) bag -> 'a [@@prototype "argMin bag"]
  ...
  (**
   * returns the element of `list` that maximizes the key extracted
   * by `f`. Used for whole-row deduplication, for example `from h in
   * horses group h.id compute (latest = (maxBy #ts) over h)`.
   *)
  val maxBy : ('a -> 'b) -> 'a list -> 'a
      [@@prototype "maxBy f list"]
  ...
  (**
   * returns the element of `list` that minimizes the key extracted
   * by `f`.
   *)
  val minBy : ('a -> 'b) -> 'a list -> 'a
      [@@prototype "minBy f list"]
